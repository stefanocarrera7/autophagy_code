    ...
    for r in [0,1,2]:
        for t in [0.2,1]:
            for g in range(1,11):
                ...
                # METRICS
                old_repo = f"{USERNAME}/autophagycode_D_he_train-mercury_{model}_strategy_trust_t{t}_g{g}_run{r}_metrics_"
                new_repo = f"{USERNAME}/autophagycode_D_he_train-mercury_{model}_strategy_trust_t{t}_g{g}_run{r}_metrics"
                try:
                    move_repo(from_id=old_repo, to_id=new_repo, repo_type="dataset", token=HF_TOKEN)
                    print(f"✅ Rinominato: {old_repo} -> {new_repo}")
                except Exception as e:
